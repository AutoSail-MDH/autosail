...
class WindError : public rclcpp::Node {
   public:
    WindError() : Node("wind_error_node") {
        ...
        subscription_ = this->create_subscription<
            autosail_message::msg::WindMessage>(  // Constructor uses the node's create_subscription class for callbacks
            param_topic_, 50, std::bind(&WindError::topic_callback, this, _1));  // No timer, instant response

        timer_ = this->create_wall_timer(  // Init timer and start timer_callback to execute 2 times/s
            500ms, std::bind(&WindError::timer_callback, this));
   ...
   private:  // Name and type must match with publisher
    void topic_callback(const autosail_message::msg::WindMessage::SharedPtr msg) {
        auto message = msg->wind_angle;
        auto wind_topic_name = subscription_->get_topic_name();

        previous_time_ = node_time_->now();  // Get time elapsed since node initialization

        if (message == FATAL) RCLCPP_FATAL(this->get_logger(), "INITIATE SHUTDOWN, %s OUT OF FUNCTION", wind_topic_name);
    }

    void timer_callback()  // Function for setting messages and publishing
    {
        auto wind_topic_name = subscription_->get_topic_name();  // param_topic_.c_str();
        ...
        if (connected == 1) {
            ...
            if (iteration >= param_iteration_)
                RCLCPP_FATAL(this->get_logger(), "INITIATE SHUTDOWN, %s OUT OF FUNCTION", wind_topic_name);
            else if ((current_time - previous_time_).seconds() < param_DL_)
                RCLCPP_WARN(this->get_logger(), "WARNING, %s MALFUNCTION", wind_topic_name);
            else
                RCLCPP_FATAL(this->get_logger(), "INITIATE SHUTDOWN, %s OUT OF FUNCTION", wind_topic_name);
        }
    }
    ...
    rclcpp::Subscription<autosail_message::msg::WindMessage>::SharedPtr subscription_;
...
int main(int argc, char* argv[]) {
    ...
    rclcpp::spin(std::make_shared<WindError>());  // Prep to receive from pub
    rclcpp::shutdown();
    return 0;
}
