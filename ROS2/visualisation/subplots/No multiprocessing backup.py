...
class MinimalPublisher(Node):
    ...
    def __init__(self):
        ...
        self.pubGPS_ = self.create_subscription(Float32MultiArray, '/wind/direction', self.wind_callback, 10)
        self.pubGPS_ # prevent unused variable warning

        self.pubGPS_ = self.create_subscription(Float32MultiArray, '/temp/rudder', self.rudder_callback, 10)
        self.pubGPS_ # prevent unused variable warning

        self.pubGPS_ = self.create_subscription(Float32MultiArray, '/temp/sail', self.sail_callback, 10)
        self.pubGPS_ # prevent unused variable warning
    ...
    def animateVel(self, i):
        ...
        self.ax1.title("Velocity")

    def animateYaw(self, i):

        #Maybe this should be elsewhere but whatever
        YawX = []

        #Set every entry in YawX to DrawYaw just to have the same dimensions
        for i in range(10):
            YawX.append(self.DrawYaw)

        self.ax2.clear()
        self.ax2.plot(YawX, self.PolarY)
    ...
    def animateWind(self, i):
        ...
        for i in range(10):
            WindX.append(self.DrawWind)

        self.ax3.clear()
        self.ax3.plot(WindX, self.PolarY)
